...
var Connection = function (options) {
   ...
   if (options && options.use_es6_promises) {
      ...
      if ('Promise' in global) {
         ...
         console.log("Warning: ES6 promises requested, but not found! Falling back to whenjs.");
...
Connection.prototype._create_transport = function () {
   for (var i = 0; i < this._transport_factories.length; ++i) {
      ...
      try {
         ...
         // ignore
         console.log("could not create WAMP transport '" + transport_factory.type + "': " + e);
      }
   }
      ...
      self._session.onjoin = function (details) {
         if (self.onopen) {
            try {
               ...
               console.log("Exception raised from app code while firing Connection.onopen()", e);
            }
         }
      };
      ...
      self._transport.onclose = function (evt) {
         ...
         if (self.onclose) {
            ...
            try {
               ...
               console.log("Exception raised from app code while firing Connection.onclose()", e);
            }
         }
         ...
         if (self._retry && !stop_retrying) {
            ...
            if (next_retry.will_retry) {
               ...
               console.log("retrying in " + next_retry.delay + " s");
               self._retry_timer = setTimeout(retry, next_retry.delay * 1000);

            } else {
               console.log("giving up trying to reconnect");
            }
         }
      }
   }
